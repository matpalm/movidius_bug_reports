...
```
mvNCCompile graph.frozen.pb -s 12 -in imgs -on output -o graph.mv
```
...
* sometimes `conv2d` with `padding='VALID'` crashes
* `deconv2d` with `padding='SAME'` is not supported ( Toolkit Error: Stage Details Not Supported: Wrong deconvolution output shape. )
