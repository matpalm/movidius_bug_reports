...
def model_for(eg):
  ...
  if eg == 'conv_with_regression':
    ...
    imgs = tf.placeholder(dtype=np.float32, shape=(1, 512, 512, 3), name='imgs')
    dump_shape_and_product_of('imgs', imgs)
    ...
    # conv layer 2 with stride 2 for downsampling
    model = slim.conv2d(model, num_outputs=32, kernel_size=3, stride=2, padding='VALID', scope='e3')
    dump_shape_and_product_of('e3', model)

    # conv layer 2 with stride 2 for downsampling
    model = slim.conv2d(model, num_outputs=64, kernel_size=3, stride=2, padding='VALID', scope='e4')
    dump_shape_and_product_of('e4', model)

    # conv layer 2 with stride 2 for downsampling
    model = slim.conv2d(model, num_outputs=128, kernel_size=3, stride=2, padding='VALID', scope='e5')
    dump_shape_and_product_of('e5', model)

    # deconv with stride 2 for upsampling
    model = slim.conv2d_transpose(model, num_outputs=64, kernel_size=3, stride=2,
                                  padding='VALID', scope='d1')
    dump_shape_and_product_of('d1', model)

    # deconv with stride 2 for upsampling
    model = slim.conv2d_transpose(model, num_outputs=32, kernel_size=3, stride=2,
                                  padding='VALID', scope='d2')
    dump_shape_and_product_of('d2', model)

    # deconv with stride 2 for upsampling
    model = slim.conv2d_transpose(model, num_outputs=16, kernel_size=3, stride=2,
                                  padding='VALID', scope='d3')
    ...
    label = tf.placeholder(dtype=np.float32, shape=(1, 127, 127, 1), name='label')
